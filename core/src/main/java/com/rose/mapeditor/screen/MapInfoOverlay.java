...
/** Read-only dialog showing metadata for the loaded map. */
public final class MapInfoOverlay extends Table {

    public MapInfoOverlay(Skin skin, String infoText) {
        setFillParent(true);
        ...
        Label titleLabel = new Label("Map Info", skin);
        ...
        panel.add(scrollPane).width(540).height(Math.min(280, Math.max(120, lineCount * 18 + 12))).left().padBottom(8).row();
    ...
    private static TextArea createReadOnlyTextArea(Skin skin, String text, int lineCount) {
        ...
        textArea.setPrefRows(Math.min(14, Math.max(4, lineCount)));
